...
if st.button("🔍 Scan ALL stocks for momentum breakouts", use_container_width=True):
    ...
    if cached:
        ...
        results.sort(key=lambda x: x.get("Score /4", 0), reverse=True)
        st.session_state["mom_results"] = results
        st.success(f"✅ Done! Found **{len(results)}** signals.")

if st.button("🗑️ Clear cache & re-scan fresh", key="clr_momentum"):
    clear_cache("momentum")
    if "mom_results" in st.session_state:
        del st.session_state["mom_results"]
    st.rerun()
