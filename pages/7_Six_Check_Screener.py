...
if st.button("🔍 Run 6-Check scan across ALL stocks", use_container_width=True):
    ...
    if cached:
        ...
        results.sort(key=lambda x: x.get("checks", 0), reverse=True)
        st.session_state["six_check_results"] = results
        st.success(f"✅ Done! Found **{len(results)}** signals.")

if st.button("🗑️ Clear cache & re-scan fresh", key="clr_sixcheck"):
    clear_cache("sixcheck")
    if "six_check_results" in st.session_state:
        del st.session_state["six_check_results"]
    st.rerun()
