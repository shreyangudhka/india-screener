...
if st.button("🔍 Scan ALL stocks for swing setups", use_container_width=True, type='primary'):
    cached = load_cached_results("swing", cache_hours=4)
    if cached:
        st.success(f"⚡ Loaded **{len(cached)}** results from cache (≤4h old). Use 'Clear cache' below to force re-scan.")
        st.session_state["swing_all"] = cached
    else:
        prog   = st.progress(0)
        status = st.empty()
...
if st.button("🗑️ Clear cache & re-scan fresh", key="clr_swing"):
    clear_cache("swing")
    if "swing_all" in st.session_state:
        del st.session_state["swing_all"]
    st.rerun()
